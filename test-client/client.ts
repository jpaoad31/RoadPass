...
async function cmdAhead(locName: string, radiusM?: number) {
  ...
  const result = await doFetch(`/hazards/ahead?${params}`) as {
    ...
    hazards: { hazard_id: string; distance_m: number; bearing_deg: number; report_count: number; confirm_count: number; reject_count: number; last_reported_at: string; confidence_score: number; confidence_tier: string; device_previously_reported: boolean }[];
  } | null;
  ...
  if (result) {
    ...
    if (result.hazards.length === 0) {
      ...
      for (const h of result.hazards) {
        ...
        const prev = h.device_previously_reported ? "SEEN" : "NEW";
        console.log(`    ${h.distance_m}m away @ ${h.bearing_deg}° | ${h.confidence_tier.toUpperCase()} (${h.confidence_score}/100) | ${prev} | ${stats}`);
